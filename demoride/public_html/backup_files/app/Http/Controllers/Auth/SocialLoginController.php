...
class SocialLoginController extends Controller
{
    ...
    public function twilio_sms(Request $request) {
        try{
            ...
            DB::table('verification_otps')->insert([
                    'otp' =>  $otp,  
                    'number' => trim($request->country).trim($request->phone)
            ]);
            $country = str_replace("+","",$request->country);


            $message = "HelloRide Provider, Verification Code is ".$otp;
            Helper::sendOtp(trim($country).trim($request->phone),$message);
            


            return response()->json("code sent");
        }
        catch (Exception $e) {
    ...
    public function twilio_code(Request $request) {
        try{
            ...
            $otp = DB::table('verification_otps')->where(
                [
                    'otp' =>  $request->code,  
                    'number' => "+".trim($country).trim($request->phone)
            ]);

            if ($otp) {
                DB::table('verification_otps')->where(
                [
                    'otp' =>  $request->code,  
                    'number' => trim($request->country).trim($request->phone)
                ])->delete();
